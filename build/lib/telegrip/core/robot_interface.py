...
class RobotROSNode(Node):
    ...
    def _gripper_angle_deg_to_main_norm(self, angle_deg: float) -> float:
        ...
        公式:
            g = 1 - (theta - theta_open) / (theta_closed - theta_open)
        其中:
            - theta 为 telegrip 内部夹爪角度（度）
            - g 为 main.py 的归一化夹爪命令，1=open, 0=close
        """
        open_angle = float(getattr(self.config, "gripper_open_angle", GRIPPER_OPEN_ANGLE))
        closed_angle = float(getattr(self.config, "gripper_closed_angle", GRIPPER_CLOSED_ANGLE))
        denom = max(abs(closed_angle - open_angle), 1e-6)
