...
def _looks_like_project_root(path: Path) -> bool:
    """
    判断一个目录是否像 telegrip 项目根目录。

    判定条件尽量保守，至少要满足前端资源和包目录都存在，
    这样安装到 site-packages 的纯包目录不会被误判为项目根。
    """
    try:
        return (
            path.is_dir()
            and (path / "telegrip").is_dir()
            and (path / "web-ui").is_dir()
...
def get_project_root() -> Path:
    ...
    if _looks_like_project_root(package_parent):
        return package_parent

    # 当 telegrip 以普通安装方式运行时，包目录通常位于 site-packages，
    # 此时需要回退到当前工作目录附近寻找真实仓库根目录。
    cwd = Path.cwd().resolve()
    for candidate in [cwd, *cwd.parents]:
        if _looks_like_project_root(candidate):
            return candidate

    # 最后兜底回退为旧逻辑，至少保持与原行为兼容。
    return package_parent
